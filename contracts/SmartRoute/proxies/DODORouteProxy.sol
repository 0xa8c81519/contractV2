...
contract DODORouteProxy is ReentrancyGuard, InitializableOwnable {
    ...
    function _multiSwap(
        ...
        for(uint256 i = 1; i < splitNumber.length; i++) { 
            ...
            uint256 curTotalAmount = IERC20(midToken[i-1]).tokenBalanceOf(assetFrom[i-1]);
            uint256 curTotalWeight = totalWeight[i-1];
            
            for(uint256 j = 0; j < splitNumber[i]; j++) {
                ...
                if(assetFrom[i] == address(this)) {
                    ...
                    IERC20(midToken[i-1]).transfer(pool, curAmount);
                }

                if(direction == 0) {
                    IDODOAdapter(adapter).sellBase(assetFrom[i+1], pool);
                } else {
                    IDODOAdapter(adapter).sellQuote(assetFrom[i+1], pool);
                }
            }
        }
    }
